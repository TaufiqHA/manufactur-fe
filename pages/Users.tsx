...
export const Users: React.FC = () => {
  const { users, currentUser, can, token, loadUsersFromAPI, addUserAPI, updateUserAPI, deleteUserAPI } = useStore();
  ...
  const [formData, setFormData] = useState<Partial<User>>({
      name: '', username: '', email: '', password: '', role: 'OPERATOR', permissions: JSON.parse(JSON.stringify(INITIAL_PERMISSIONS))
  });

  // Load users from API when token is available
  useEffect(() => {
    if (!token) {
      // If no token, set loading to false and clear users
      setLoading(false);
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        if (editingId) {
          ...
          await updateUserAPI({ ...formData, id: editingId } as User);
        } else {
          // Create new user
          await addUserAPI(formData as User);
        }
  ...
  return (
    <div className="space-y-8">
       <div className="flex justify-between items-center">
        ...
        <button onClick={() => { setEditingId(null); setFormData({name: '', username: '', email: '', password: '', role: 'OPERATOR', permissions: JSON.parse(JSON.stringify(INITIAL_PERMISSIONS))}); setIsModalOpen(true); }} className="bg-slate-900 text-white px-8 py-3.5 rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl">+ TAMBAH USER</button>
      </div>
                ...
                {paginatedUsers.length > 0 ? (
                  paginatedUsers.map(user => (
                     <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="px-8 py-5 text-blue-600 font-black tracking-widest text-xs">@{user.email || user.username}</td>
                  ...
                  <tr>
                    <td colSpan={5} className="px-8 py-12 text-center text-slate-500 font-bold">
                      {loading ? 'Memuat data...' : 'Tidak ada data pengguna ditemukan'}
                    </td>
                  </tr>
                )}
             </tbody>
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-10 space-y-10">
                    <form id="userForm" onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                       ...
                       <div className="space-y-2">
                          ...
                          <input required type="email" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-black outline-none" value={formData.email || formData.username} onChange={e => setFormData({...formData, email: e.target.value, username: e.target.value})} />
                       </div>
                       <div className="space-y-2">
                       ...
                       <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Password {editingId ? '(Kosongkan jika tidak ingin diubah)' : ''}</label>
                          <input type="password" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-black outline-none" placeholder={editingId ? "Kosongkan jika tidak ingin diubah" : "Masukkan password"} onChange={e => setFormData({...formData, password: e.target.value})} />
                       </div>
                    </form>
