...
import { Plus, Trash2, Wrench, X, ChevronLeft, ChevronRight, Search, Settings2, AlertCircle, Loader2 } from 'lucide-react';
import { Machine, ALL_STEPS, Shift } from '../types';

export const Machines: React.FC = () => {
  const { machines, addMachine, updateMachine, deleteMachine, toggleMaintenance, can, users, loadMachines, loadUsersFromAPI } = useStore();
  ...
  // Load machines and users when component mounts
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setError('Gagal memuat data mesin dan pengguna. Silakan coba lagi nanti.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateMachine({ ...formData, id: editingId } as Machine);
      } else {
        await addMachine({ ...formData, id: `mac-${Date.now()}`, status: 'IDLE', personnel: formData.personnel || [], isMaintenance: false } as Machine);
      }
      setIsModalOpen(false);
    } catch (err) {
      setError('Gagal menyimpan data mesin. Silakan coba lagi.');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setError('Gagal menghapus mesin. Silakan coba lagi.');
  ...
  const handleToggleMaintenance = async (id: string) => {
    ...
    try {
      ...
      setError('Gagal mengubah status perawatan. Silakan coba lagi.');
    ...
    return (
      <div className="flex justify-center items-center h-64">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-2 text-slate-600 font-bold">Memuat data mesin...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        ...
        <button
          onClick={() => {
            setEditingId(null);
            setFormData({code:'', name:'', type:'POTONG', capacityPerHour:0, personnel:[]});
            setIsModalOpen(true);
          }}
          className="bg-slate-900 text-white px-8 py-3.5 rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl"
          disabled={loading}
        >
          + TAMBAH MESIN
        </button>
      ...
      <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden">
                ...
                {paginatedMachines.map(m => (
                   <tr key={m.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-8 py-5">
                         ...
                         {m.personnel.length === 0 && <div className="text-slate-400 text-xs italic">Tidak ada PIC</div>}
                      </td>
        ...
        {paginatedMachines.length === 0 && (
          <div className="text-center py-12">
            <p className="text-slate-500 font-bold">Tidak ada data mesin ditemukan</p>
          </div>
        )}
      </div>
           ...
           <div className="bg-white rounded-[32px] md:rounded-[48px] shadow-2xl w-full max-w-2xl max-h-[95vh] flex flex-col overflow-hidden animate-in zoom-in-95">
                ...
                <form id="machineForm" onSubmit={handleSubmit} className="space-y-8">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      ...
                      <div className="space-y-2">
                         ...
                         <input
                           ...
                           onChange={e => setFormData({...formData, capacityPerHour: Number(e.target.value)})}
                           disabled={loading}
                         />
                      </div>
                   </div>
                   ...
                   <div className="space-y-6">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                         ...
                         <button
                           type="button"
                           onClick={() => setFormData(p => ({...p, personnel: [...(p.personnel || []), {id: `p-${Date.now()}`, name: '', role: 'OPERATOR', shift: 'SHIFT_1'}]}))}
                           className="text-[9px] font-black bg-slate-900 text-white px-5 py-2 rounded-xl shadow-lg hover:bg-blue-600 transition-all"
                           disabled={loading}
                         >
                           + TAMBAH OPERATOR
                         </button>
                         ...
                         {(formData.personnel || []).map(p => (
                            <div key={p.id} className="grid grid-cols-12 gap-3 items-center bg-slate-50 p-4 rounded-[28px] border border-white shadow-inner">
                               <div className="col-span-12 sm:col-span-5">
                                 <select
                                   required
                                   className="w-full p-3 bg-white rounded-xl text-xs font-black outline-none border border-slate-100"
                                   value={p.name}
                                   onChange={e => setFormData(s => ({...s, personnel: (s.personnel || []).map(x => x.id === p.id ? {...x, name: e.target.value} : x)}))}
                                   disabled={loading}
                                 >
                               ...
                               <div className="col-span-6 sm:col-span-3">
                                 <select
                                   className="w-full p-3 bg-white rounded-xl text-xs font-black outline-none border border-slate-100"
                                   value={p.shift}
                                   onChange={e => setFormData(s => ({...s, personnel: (s.personnel || []).map(x => x.id === p.id ? {...x, shift: e.target.value as Shift} : x)}))}
                                   disabled={loading}
                                 >
                               ...
                               <div className="col-span-4 sm:col-span-3">
                                 <select
                                   className="w-full p-3 bg-white rounded-xl text-xs font-black outline-none border border-slate-100"
                                   value={p.role}
                                   onChange={e => setFormData(s => ({...s, personnel: (s.personnel || []).map(x => x.id === p.id ? {...x, role: e.target.value as any} : x)}))}
                                   disabled={loading}
                                 >
                               ...
                               <button
                                 type="button"
                                 className="col-span-2 sm:col-span-1 flex justify-center text-red-400 hover:text-red-600 transition-colors"
                                 onClick={() => setFormData(prev => ({...prev, personnel: prev.personnel?.filter(x => x.id !== p.id)}))}
                                 disabled={loading}
                               >
                                 <Trash2 size={18}/>
                               </button>
              ...
              <div className="p-6 md:p-10 border-t shrink-0 bg-slate-50">
                <button
                  type="submit"
                  form="machineForm"
                  className="w-full py-5 bg-blue-600 text-white rounded-[24px] font-black shadow-2xl hover:bg-blue-700 transition-all text-base uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed"
                  disabled={loading}
                >
                  {loading ? (
